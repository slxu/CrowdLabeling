...
public class AceReader extends GenericDataSetReader {
	...
	public static String aceVersion = "ACE2004";
	...
	/**
	 * Reads in ACE*.apf.xml files and converts them to RelationSentence
	 * objects. Note that you probably should call parse() instead.
	 * 
	 * Currently, this ignores document boundaries (the list returned will
	 * include sentences from all documents).
	 * 
	 * @param path
	 *            directory containing ACE files to read (e.g.
	 *            "/home/mcclosky/scr/data/ACE2005/english_test"). This can also
	 *            be the path to a single file. *
	...
	private void countNameRelations(CoreMap sent) {
		...
		if (mentions != null) {
			for (RelationMention m : mentions) {
				...
				if (args.size() == 2 && args.get(0).getMentionType().equals("NAM")
						&& args.get(1).getMentionType().equals("NAM")) {
	...
	private List<CoreMap> readDocument(String prefix, Annotation corpus) throws IOException, SAXException,
		...
		/*
		 * for (int sentenceIndex = 0; sentenceIndex <
		 * aceDocument.getSentenceCount(); sentenceIndex++) { List<AceToken>
		 * tokens = aceDocument.getSentence(sentenceIndex); StringBuffer b = new
		 * StringBuffer(); for(AceToken t: tokens) b.append(t.getLiteral() + " "
		...
		for (int sentenceIndex = 0; sentenceIndex < aceDocument.getSentenceCount(); sentenceIndex++) {
			...
			// skip "sentences" that are really just SGML tags (which come from
			// using the RobustTokenizer)
			if (words.size() == 1) {
			...
			for (AceEntityMention aceEntityMention : entityMentions) {
				...
				EntityMention convertedMention = convertAceEntityMention(aceEntityMention, docId, sentence,
						tokenOffset, corefID);
				// EntityMention convertedMention =
				// convertAceEntityMention(aceEntityMention, docId, sentence,
				// tokenOffset);
			...
			for (AceRelationMention aceRelationMention : relationMentions) {
				RelationMention convertedMention = convertAceRelationMention(aceRelationMention, docId, sentence,
						entityMentionMap);
			...
			for (AceEventMention aceEventMention : eventMentions) {
				EventMention convertedMention = convertAceEventMention(aceEventMention, docId, sentence,
						entityMentionMap, tokenOffset);
				if (convertedMention != null) {
	...
	private RelationMention convertAceRelationMention(AceRelationMention aceRelationMention, String docId,
		...
		// the arguments are already stored in semantic order. Make sure we
		...
		if (argNames.size() != 2 || !argNames.get(0).equalsIgnoreCase("arg-1")
				|| !argNames.get(1).equalsIgnoreCase("arg-2")) {
	 ...
	 */
	private EntityMention convertAceEntityMention(AceEntityMention entityMention, String docId, CoreMap sentence,
			int tokenOffset) {
		// System.err.println("TYPE is " + entityMention.getParent().getType());
		// System.err.println("SUBTYPE is " +
		...
		if (extStart < 0) {
			logger.severe("READER ERROR: Invalid extent start " + extStart + " for entity mention "
					+ entityMention.getId() + " in document " + docId + " in sentence " + sentence);
		...
		if (extEnd > sentence.get(CoreAnnotations.TokensAnnotation.class).size()) {
			logger.severe("READER ERROR: Invalid extent end " + extEnd + " for entity mention " + entityMention.getId()
					+ " in document " + docId + " in sentence " + sentence);
			logger.severe("This may happen due to incorrect EOS detection. Adjusting entity extent.");
		...
		if (headStart < 0) {
			logger.severe("READER ERROR: Invalid head start " + headStart + " for entity mention "
					+ entityMention.getId() + " in document " + docId + " in sentence " + sentence);
		...
		if (headEnd > sentence.get(CoreAnnotations.TokensAnnotation.class).size()) {
			logger.severe("READER ERROR: Invalid head end " + headEnd + " for entity mention " + entityMention.getId()
					+ " in document " + docId + " in sentence " + sentence);
			logger.severe("This may happen due to incorrect EOS detection. Adjusting entity head span.");
		...
		// note: the ace.reader stores absolute token offset from the beginning
		// of the document, but
		// we need token offsets from the beginning of the sentence => adjust by
		// tokenOffset
	...
	private EntityMention convertAceEntityMention(AceEntityMention entityMention, String docId, CoreMap sentence,
			int tokenOffset, String corefID) {
	...
	public static void main(String[] args) throws Exception {
		...
		Annotation a = r
				.parse("C:/Users/clzhang/Downloads/ace2004/ACE2004/data/English/nwire_tiny/");
...
// ParserConfigurationException {
//
// AceReader ar = new AceReader();
// Annotation an = ar.read(path);
// D.p(an.toString());
// }
// }
