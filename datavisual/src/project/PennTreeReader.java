...
/**
 * This class implements the <code>TreeReader</code> interface to read Penn
 * Treebank-style files. The reader is implemented as a pushdown automaton (PDA)
 * that parses the Lisp-style format in which the trees are stored. This reader
 * is compatible with both PTB and PATB trees.
 * 
...
public class PennTreeReader implements TreeReader {
	...
	// misuse a list as a stack, since we want to avoid the synchronized and old
	// Stack, but don't need the power and JDK 1.6 dependency of a Deque
	private ArrayList<Tree> stack;
	private static final String leftParen = "(";
	private static final String rightParen = ")";

	/**
	 * Read parse trees from a <code>Reader</code>. For the defaulted arguments,
	 * you get a <code>SimpleTreeFactory</code>, no <code>TreeNormalizer</code>,
	 * and a <code>PennTreebankTokenizer</code>.
	 * 
	 * @param in
	 ...
	 *            the Reader
	 * @param tf
	 *            TreeFactory -- factory to create some kind of Tree
	 */
	 ...
	 *            TreeFactory -- factory to create some kind of Tree
	 * @param tn
	 *            the method of normalizing trees
	 */
	 ...
	 *            the method of normalizing trees
	 * @param st
	 *            Tokenizer that divides up Reader
	 */
	public PennTreeReader(Reader in, TreeFactory tf, TreeNormalizer tn, Tokenizer<String> st) {
		...
		// check for whacked out headers still present in Brown corpus in
		// Treebank 3
	...
	/**
	 * Reads a single tree in standard Penn Treebank format from the input
	 * stream. The method supports additional parentheses around the tree (an
	 * unnamed ROOT node) so long as they are balanced. If the token stream ends
	 * before the current tree is complete, then the method will throw an
	 * <code>IOException</code>.
	 * <p>
	 * Note that the method will skip malformed trees and attempt to read
	 * additional trees from the input stream. It is possible, however, that a
	 * malformed tree will corrupt the token stream. In this case, an
	 * <code>IOException</code> will eventually be thrown.
	 * 
	...
	public Tree readTree() throws IOException {
		...
		while (tokenizer.hasNext() && t == null) {
			...
			if (t != null) {
				// cdm 20100618: Don't do this! This was never the historical
				// behavior!!!
				// Escape empty trees e.g. (())
				// while(t != null && (t.value() == null ||
				// t.value().equals("")) && t.numChildren() <= 1)
				// t = t.firstChild();
	...
	private Tree getTreeFromInputStream() throws NoSuchElementException {
		...
		while (tokenizer.hasNext()) {
			...
			if (token.equals(leftParen)) {
				...
				// cdm 20100225: This next line used to have "" instead of null,
				// but the traditional and current tree normalizers depend on
				// the label being null not "" when there is no label on a tree
				// (like the outermost English PTB level)
				...
				Tree newTree = treeFactory.newTreeNode(label, null); // dtrs are
																		// added
																		// below
				if (currentTree == null)
					stack.add(newTree);
				...
				if (stack.isEmpty()) {
					...
					System.err
							.println("PennTreeReader: warning: file has extra non-matching right parenthesis [ignored]");
					break;
				}

				// Accept
				currentTree = stack.remove(stack.size() - 1); // i.e.,
																// stack.pop()
				if (stack.isEmpty())
					return currentTree;
				...
				if (currentTree == null) {
					// A careful Reader should warn here, but it's kind of
					// useful to
					// suppress this because then the TreeReader doesn't print a
					// ton of
					// messages if there is a README file in a directory of
					// Trees.
					// System.err.println("PennTreeReader: warning: file has extra token not in a s-expression tree: "
					// + token + " [ignored]");
					break;
				}
				...
				// cdm: Note: this implementation just isn't as efficient as the
				// old recursive descent parser (see 2008 code), where all the
				// daughters are gathered before the tree is made....
			}
		}

		// Reject
		if (currentTree != null) {
			System.err.println("PennTreeReader: warning: incomplete tree (extra left parentheses in input): "
					+ currentTree);
		}
		return null;
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			Reader r = new BufferedReader(new InputStreamReader(new FileInputStream(penntreedir+"/wsj_0003.mrg"), "UTF-8"));
