...
\title{Visualizing NLP annotations for Crowds }
...
\begin{abstract}
Visualizing NLP annotation is essential to collect large amounts of training data for statistical NLP methods.  Existing toolkits often provide comprehensive operators to allow experts to realize sophisticated linguistic rules. Users must be well trained to use them. So they can hardly scale to large amounts of non-expert crowd-sourced workers. In this paper, we present \sys, a visualization toolkit to allow crowd-sourced workers to annotate two general categories of NLP problems: clustering and parsing. Workers can finish the tasks with intuitive operators in an interactive interface, and fix errors conveniently. User studies show our toolkit is very friendly to nonexperts of NLP, and allow them to produce high quality labeled NLP data for several sophisticated NLP problems. We release our source code and toolkit to spur future research. 
\end{abstract}
